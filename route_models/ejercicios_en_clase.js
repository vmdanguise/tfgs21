...
router.get("/", function (req, res, next) {
  sql = "SELECT * from public.ejercicios_en_clase";
...
router.post("/update_ejercicios_en_clase", async function (req, res) {
  ...
  let sql = "UPDATE public.ejercicios_en_clase SET idejercicios='"+req.body.idejercicios+"' ,idClase='"+req.body.idClase+"' WHERE idejercicios='"+req.body.idejercicios+"'"; 
   connection.query(sql, function (err, result, rows) {
     if (err) {
       res.render("error",{layout:'default',erroresporparametro:err});
       return null;
...
router.post("/insert_ejercicios_en_clase", function (req, res) {
  let sql = "INSERT INTO public.ejercicios_en_clase (idejercicios ,idClase ) values ('"+req.body.idejercicios+"' ,'" + req.body.idClase + "')";
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
      return null;
...
router.post("/delete_ejercicios_en_clase", function (req, res) {
  let sql = "DELETE from public.ejercicios_en_clase WHERE idejercicios = '" + req.body.idejercicios + "'";
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
      return null;
...
router.post("/select_ejercicios_en_clase", function (req, res) {
  ...
  if (1 == 1  && req.body.idejercicios == ""  && req.body.idClase == "" ) {
    ...
    sql = "SELECT * from public.ejercicios_en_clase WHERE 1=1  and idejercicios like '%" + req.body.idejercicios + "%'  and idClase like '%" + req.body.idClase + "%' ;"
  }
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
