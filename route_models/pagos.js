...
router.post("/update_pagos", async function (req, res) {
  ...
  let sql = "UPDATE public.pagos SET idpagos='"+req.body.idpagos+"' ,fecha='"+req.body.fecha+"',idAlumno='"+req.body.idAlumno+"' WHERE idpagos='"+req.body.idpagos+"'"; 
   connection.query(sql, function (err, result, rows) {
     if (err) {
       res.render("error",{layout:'default',erroresporparametro:err});
       return null;
...
router.post("/insert_pagos", function (req, res) {
  let sql = "INSERT INTO public.pagos (idpagos ,fecha ,idAlumno ) values ('"+req.body.idpagos+"' ,'" + req.body.fecha + "','" + req.body.idAlumno + "')";
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
      return null;
...
router.post("/delete_pagos", function (req, res) {
  let sql = "DELETE from public.pagos WHERE idpagos = '" + req.body.idpagos + "'";
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
      return null;
...
router.post("/select_pagos", function (req, res) {
  ...
  if (1 == 1  && req.body.idpagos == ""  && req.body.fecha == ""  && req.body.idAlumno == "" ) {
    ...
    sql = "SELECT * from public.pagos WHERE 1=1  and idpagos like '%" + req.body.idpagos + "%'  and fecha like '%" + req.body.fecha + "%'  and idAlumno like '%" + req.body.idAlumno + "%' ;"
  }
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
