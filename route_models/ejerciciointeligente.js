...
router.post('/update_ejerciciointeligente', async function (req, res) {
  ...
  let sql = 'UPDATE pilates_virtual_class.ejerciciointeligente SET idejercicioInteligente="'+req.body.idejercicioInteligente+'" ,tipo="'+req.body.tipo+'",escuela="'+req.body.escuela+'",asistenteVirtual="'+req.body.asistenteVirtual+'" WHERE idejercicioInteligente="'+req.body.idejercicioInteligente+'"'; 
   connection.query(sql, function (err, result, rows) {
     if (err) {
       res.render('error',{layout:"default",erroresporparametro:err});
       return null;
...
router.post('/insert_ejerciciointeligente', function (req, res) {
  let sql = 'INSERT INTO pilates_virtual_class.ejerciciointeligente (idejercicioInteligente ,tipo ,escuela ,asistenteVirtual ) values ("'+req.body.idejercicioInteligente+'" ,"' + req.body.tipo + '","' + req.body.escuela + '","' + req.body.asistenteVirtual + '")';
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render('error',{layout:"default",erroresporparametro:err});
      return null;
...
router.post('/delete_ejerciciointeligente', function (req, res) {
  let sql = 'DELETE from pilates_virtual_class.ejerciciointeligente WHERE idejercicioInteligente = "' + req.body.idejercicioInteligente + '"';
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render('error',{layout:"default",erroresporparametro:err});
      return null;
...
router.post('/select_ejerciciointeligente', function (req, res) {
  ...
  if (1 == 1  && req.body.idejercicioInteligente == ''  && req.body.tipo == ''  && req.body.escuela == ''  && req.body.asistenteVirtual == '' ) {
    ...
    sql = 'SELECT * from pilates_virtual_class.ejerciciointeligente WHERE 1=1  and idejercicioInteligente like "%' + req.body.idejercicioInteligente + '%"  and tipo like "%' + req.body.tipo + '%"  and escuela like "%' + req.body.escuela + '%"  and asistenteVirtual like "%' + req.body.asistenteVirtual + '%" ;'
  }
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render('error',{layout:"default",erroresporparametro:err});
