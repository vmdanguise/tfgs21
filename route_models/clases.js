...
router.post("/update_clases", async function (req, res) {
  ...
  let sql = "UPDATE public.clases SET idClases='"+req.body.idClases+"' ,fecha='"+req.body.fecha+"',actividades='"+req.body.actividades+"',id_profesor='"+req.body.id_profesor+"' WHERE idClases='"+req.body.idClases+"'"; 
   connection.query(sql, function (err, result, rows) {
     if (err) {
       res.render("error",{layout:'default',erroresporparametro:err});
       return null;
...
router.post("/insert_clases", function (req, res) {
  let sql = "INSERT INTO public.clases (idClases ,fecha ,actividades ,id_profesor ) values ('"+req.body.idClases+"' ,'" + req.body.fecha + "','" + req.body.actividades + "','" + req.body.id_profesor + "')";
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
      return null;
...
router.post("/delete_clases", function (req, res) {
  let sql = "DELETE from public.clases WHERE idClases = '" + req.body.idClases + "'";
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
      return null;
...
router.post("/select_clases", function (req, res) {
  ...
  if (1 == 1  && req.body.idClases == ""  && req.body.fecha == ""  && req.body.actividades == ""  && req.body.id_profesor == "" ) {
    ...
    sql = "SELECT * from public.clases WHERE 1=1  and idClases like '%" + req.body.idClases + "%'  and fecha like '%" + req.body.fecha + "%'  and actividades like '%" + req.body.actividades + "%'  and id_profesor like '%" + req.body.id_profesor + "%' ;"
  }
  connection.query(sql, function (err, result, rows) {
    if (err) {
      res.render("error",{layout:'default',erroresporparametro:err});
